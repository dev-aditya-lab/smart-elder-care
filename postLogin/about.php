<!DOCTYPE html>
<html lang="en">

<head>
    <meta charset="UTF-8">
    <meta name="viewport" content="width=device-width, initial-scale=1.0">
    <title>Document</title>
    <title>Smart Elder Care</title>

    <script>
        addEventListener("load", function () {
            setTimeout(hideURLbar, 0);
        }, false);

        function hideURLbar() {
            window.scrollTo(0, 1);
        }
    </script>
    <!-- Custom-Files -->
    <link rel="stylesheet" href="css/bootstrap.css">
    <!-- Bootstrap-Core-CSS -->
    <link rel="stylesheet" href="css/style.css" type="text/css" media="all" />
    <!-- Style-CSS -->
    <link rel="stylesheet" href="css/font-awesome.css">
    <!-- Font-Awesome-Icons-CSS -->
    <!-- Web-Fonts -->
    <link
        href="//fonts.googleapis.com/css?family=Lato:100,100i,300,300i,400,400i,700,700i,900,900i&amp;subset=latin-ext"
        rel="stylesheet">
    <link
        href="//fonts.googleapis.com/css?family=Roboto:100,100i,300,300i,400,400i,500,500i,700,700i,900,900i&amp;subset=cyrillic,cyrillic-ext,greek,greek-ext,latin-ext,vietnamese"
        rel="stylesheet">
    <!-- //Web-Fonts -->
</head>

<body>
    <!-- header -->
    <header>
        <!-- navigation -->
        <div class="main-top py-1">
            <nav class="navbar navbar-expand-lg navbar-light fixed-navi">
                <div class="container-fluid">
                    <!-- logo -->
                    <a class="navbar-brand font-weight-bold" href="index.php">
                        Smart Elder Care
                    </a>
                    <!-- //logo -->
                    <button class="navbar-toggler" type="button" data-toggle="collapse"
                        data-target="#navbarSupportedContent" aria-controls="navbarSupportedContent"
                        aria-expanded="false" aria-label="Toggle navigation">
                        <span class="navbar-toggler-icon"></span>
                    </button>
                    <div class="collapse navbar-collapse text-center" id="navbarSupportedContent">
                        <ul class="navbar-nav ml-lg-auto">
                            <li class="nav-item ">
                                <a class="nav-link" href="index.php">Home
                                    <span class="sr-only">(current)</span>
                                </a>
                            </li>
                            <li class="nav-item mx-xl-5 mx-lg-3 my-lg-0 my-3 active">
                                <a class="nav-link" href="about.php">About Us</a>
                            </li>
                            <li class="nav-item ">
                                <a class="nav-link " href="index.php">
                                    Our Services
                                </a>
                            </li>
                            <li class="nav-item mx-xl-5 mx-lg-3 mt-lg-0 mt-3">
                                <a class="nav-link" href="sub&price.php">Pricing and Subscriptions</a>
                            </li>
                            <li class="nav-item mt-lg-0 mt-3 mb-3">
                                <a class="nav-link" href="contact/contect.php">Contact Us</a>
                            </li>
                            <!--  -->
                            <li class="ml-lg-4 mb-lg-0 mb-4">
                                <a href="login/login.php" class="reqe-button">Login</a>
                            </li>

                            <li class="ml-lg-4 mb-lg-0 mb-4">
                                <a href="sign up/index.php" class="reqe-button">Join as Halper!</a>
                            </li>
                        </ul>
                    </div>
                </div>
            </nav>
        </div>
        <!-- //navigation -->
    </header>
    <!-- //header -->


    <!-- About -->
    <div class="middle-w3l pb-5 bg-light" id="some">
        <div class=" pb-xl-5 pb-lg-3">
            <div class="">
                <div class="left-build-wthree">
                    <h2 class="middle-title-w3 text-dark font-weight-bold">About us!</h2>
                    <h6 class="mt-5">At Smart Elder Care, we understand the importance of providing the best care and companionship for your aging
                        parents. Our journey began with a simple yet powerful idea: to make it easier for children to support and care for their
                        elderly loved ones.</h6>
                        <h6 class="mt-5">
                        <h2 class=" text-dark font-weight-bold">Our Mission</h2>
                        Our mission is to enhance the quality of life for seniors and their families by connecting them with compassionate and
                        reliable helpers. We believe that every elderly individual deserves to age gracefully in the comfort of their own home,
                        surrounded by caring companions.</h6>
                        <br><hr>
                        <h2 class=" text-dark font-weight-bold">Why Choose Us</h2>
                    <div class="row">
                        <div class="col-sm-6">
                            <ul class="list-beach list-unstyled mt-lg-5 mt-4">
                                <li>
                                    <div class="row">
                                        <div class="col-2 text-center pr-sm-0">
                                            <i class="fa fa-long-arrow-right" aria-hidden="true"></i>
                                        </div>
                                        <div class="col-10">
                                            <p><span class="about-point">Experienced Caregivers:</span> We carefully select and train our caregivers, ensuring they are not only skilled but also
                                            compassionate and dedicated to improving the lives of seniors.</p>
                                        </div>
                                    </div>
                                </li>
                                <li class="my-3">
                                    <div class="row">
                                        <div class="col-2 text-center pr-sm-0">
                                            <i class="fa fa-long-arrow-right" aria-hidden="true"></i>
                                        </div>
                                        <div class="col-10">
                                            <p><span class="about-point">Personalized Care: </span>We understand that every senior has unique needs, and we tailor our services to meet those needs,
                                            providing personalized care plans that foster well-being.</p>
                                        </div>
                                    </div>
                                </li>
                      
                            </ul>
                        </div>

                        <div class="col-sm-6">
                            <ul class="list-beach list-unstyled mt-lg-5 mt-sm-4 mt-3">
                                <li>
                                    <div class="row">
                                        <div class="col-2 text-center pr-sm-0">
                                            <i class="fa fa-long-arrow-right" aria-hidden="true"></i>
                                        </div>
                                        <div class="col-10">
                                            <p><span class="about-point">Peace of Mind:</span> We provide peace of mind to families, knowing their loved ones are in good hands. Our helpers offer not
                                            only assistance but also companionship, creating meaningful connections.</p>
                                        </div>
                                    </div>
                                </li>

                                <li class="my-3">
                                    <div class="row">
                                        <div class="col-2 text-center pr-sm-0">
                                            <i class="fa fa-long-arrow-right" aria-hidden="true"></i>
                                        </div>
                                        <div class="col-10">
                                            <p><span class="about-point">Convenience:</span>
                                                 Our subscription model is designed to be easy and convenient for you. We take care of the logistics, so you
                                            can focus on what matters most – spending time with your parents.</p>
                                        </div>
                                    </div>
                                </li>
    
                            </ul>
                        </div>
                    </div>
                    <!-- <a href="blog.php" class="btn button-style mt-sm-5 mt-4">Read More</a> -->
                    <h2 class=" text-dark font-weight-bold">Join Us in Our Mission</h2>
                    <h6 class="mt-5">We invite you to join us in our mission to ensure that aging parents receive the support and companionship they deserve.
                    Together, we can make the senior years truly golden.
                    <br>
                    Thank you for considering Smart Elder Care for your elderly loved ones' care needs. We look forward to serving your
                    family with dedication and compassion.</h6>
                </div>
                <div class="right-side-img-w3">
    
                </div>
                <div class="clearfix"></div>
            </div>
        </div>
    </div>
    <!-- //About -->


    <!-- footer -->
    <footer>
        <div class="w3ls-footer-grids py-4">
            <div class="container py-xl-5 py-lg-3">
                <div class="row">
                    <div class="col-lg-3 w3l-footer-logo text-center">
                        <!-- logo -->
                        <a class="navbar-brand font-weight-bold" href="index.php">
                            Smart Elder Care
                        </a>
                        <!-- //logo -->
                    </div>
                    <!-- button -->
                    <div class="col-lg-5 w3l-footer text-lg-right text-center mt-3">
                        <ul class="list-unstyled footer-nav-wthree">
                            <li class="mr-4">
                                <a href="index.php" class="active">Home</a>
                            </li>
                            <li class="mr-4">
                                <a class="" href="about.php">About Us</a>
                            </li>
                            <li>
                                <a class="" href="contact.php">Contact Us</a>
                            </li>
                        </ul>
                    </div>
                    <!-- //button -->
                    <!-- social icons -->
                    <div class="col-lg-4 w3social-icons text-lg-right text-center mt-lg-0 mt-3">
                        <ul>
                            <li>
                                <a href="#" class="rounded-circle">
                                    <i class="fa fa-facebook-f"></i>
                                </a>
                            </li>
                            <li class="px-2">
                                <a href="#" class="rounded-circle">
                                    <i class="fa fa-google-plus"></i>
                                </a>
                            </li>
                            <li>
                                <a href="#" class="rounded-circle">
                                    <i class="fa fa-twitter"></i>
                                </a>
                            </li>
                        </ul>
                    </div>
                    <!-- //social icons -->
                </div>
                <div class="pt-lg-4 pt-3 text-center">
                    <!-- copyright -->
                    <p class="copy-right-grids mt-lg-1">© 2021 Smart Elder Care | All Rights Reserved | Design by
                        <a href="#" target="_blank">Ishika </a>
                    </p>
                    <!-- //copyright -->
                </div>
            </div>
        </div>
    </footer>
    <!-- //footer -->


    <!-- Js files -->
    <!-- JavaScript -->
    <script src="js/jquery-2.2.3.min.js"></script>
    <!-- Default-JavaScript-File -->
    <script src="js/bootstrap.js"></script>
    <!-- Necessary-JavaScript-File-For-Bootstrap -->

    <!-- fixed navigation -->
    <script src="js/fixed-nav.js"></script>
    <!-- //fixed navigation -->
    <!-- dropdown smooth -->
    <script>
        $(document).ready(function () {
            $(".dropdown").hover(
                function () {
                    $('.dropdown-menu', this).stop(true, true).slideDown("fast");
                    $(this).toggleClass('open');
                },
                function () {
                    $('.dropdown-menu', this).stop(true, true).slideUp("fast");
                    $(this).toggleClass('open');
                }
            );
        });
    </script>
    <!-- //dropdown smooth -->

    <!-- search plugin -->
    <!-- pop-up-box -->
    <link href="css/popuo-box.css" rel="stylesheet" type="text/css" media="all" />
    <script src="js/jquery.magnific-popup.js"></script>
    <!-- //pop-up-box -->
    <!-- search script -->
    <script>
        $(document).ready(function () {
            $('.popup-with-zoom-anim').magnificPopup({
                type: 'inline',
                fixedContentPos: false,
                fixedBgPos: true,
                overflowY: 'auto',
                closeBtnInside: true,
                preloader: false,
                midClick: true,
                removalDelay: 300,
                mainClass: 'my-mfp-zoom-in'
            });

        });
    </script>
    <!-- //search script -->
    <!-- //search plugin -->

    <!-- smooth scrolling -->
    <script src="js/SmoothScroll.min.js"></script>
    <!-- move-top -->
    <script src="js/move-top.js"></script>
    <!-- easing -->
    <script src="js/easing.js"></script>
    <!--  necessary snippets for few javascript files -->
    <script src="js/inside.js"></script>

    <script src="js/bootstrap.js"></script>
    <!-- Necessary-JavaScript-File-For-Bootstrap -->
    <!-- //Js files -->

</body>

</html>
